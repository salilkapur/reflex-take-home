...
# Database instance with connection pooling (pool size: 15)
db = ClickHouseVideoDatabase(pool_size=15)
...
async def list_videos(
    ...
    try:
        ...
        if include_unprocessed:
            ...
            file_unprocessed_videos = await asyncio.to_thread(scan_data_folder_for_videos)
...
async def get_video_episodes(video_id: str):
    episodes = await asyncio.to_thread(db.get_episodes_for_video, video_id)
